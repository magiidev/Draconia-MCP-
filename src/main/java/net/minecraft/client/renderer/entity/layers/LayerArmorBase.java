...
public abstract class LayerArmorBase<T extends ModelBase> implements LayerRenderer<EntityLivingBase>
{
    ...
    private ResourceLocation getArmorResource(ItemArmor armorItem, boolean isLeggings, String type)
    {
        String materialName = armorItem.getArmorMaterial().getName();
        String namespace = "minecraft";

        if (materialName.startsWith(Constants.NAME.toLowerCase() + ":"))
        {
            namespace = Constants.NAME.toLowerCase();
    ...
    public ResourceLocation getArmorResource(Entity entity, ItemStack stack, int slot, String type)
    {
        ...
        if (materialName.startsWith(Constants.NAME.toLowerCase() + ":"))
        {
            namespace = Constants.NAME.toLowerCase(); // Utiliser le namespace de ton mod
            materialName = materialName.substring(materialName.indexOf(':') + 1); // Enlève le namespace
        }

        String texturePath = String.format("%s:textures/models/armor/%s_layer_%d%s.png",
                namespace,
                materialName,
                this.isSlotForLeggings(slot) ? 2 : 1,
                type == null ? "" : "_" + type);
